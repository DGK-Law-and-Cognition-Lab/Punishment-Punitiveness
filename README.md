...
| **NLP — Preprocessing & Dictionaries** | Phase 1 of the Python NLP pipeline · [Open in Colab](https://colab.research.google.com/drive/1oQSBmuGCB48b5hF1DWkte1qpHw9etV0X) |
| **NLP — Classification** | Phase 2: DeBERTa zero-shot, Claude forced-choice · [Open in Colab](https://colab.research.google.com/drive/1Qjk3VlOik9ObHZmyDurYIfFaUzYhpGJ0) |
| **NLP — Embeddings & Similarity** | Phase 3: BGE, voyage, MPNet — five prototype sets, 45 combinations · [Open in Colab](https://colab.research.google.com/drive/11Dpj5cgn0LTxxwjn7DMI-sZvj1F4wOJC) |
| **NLP — Topics, Convergence & Export** | Phase 4: BERTopic, cross-method convergence, final feature export · [Open in Colab](https://colab.research.google.com/drive/1JJDWBjEQO-dCKisftIBXK--E7HN-BGKT) |

### Key Visualizations

| Figure | Description |
|--------|-------------|
| **[Cluster-Level Heatmap](analysis/output/figures/main/heatmap_cluster_level.png)** | Punitiveness × correlate-cluster correlations |
| **[Construct-Level Heatmap](analysis/output/figures/main/heatmap_construct_level.png)** | Punitiveness × all 18 constructs |
| **[Sentencing Distribution](analysis/output/figures/main/sentence_histogram_overall.png)** | Distribution of sentencing decisions vs. 25-year anchor |
| **[Sentencing by Vignette](analysis/output/figures/main/sentence_histogram_by_vignette.png)** | Per-vignette sentencing distributions |
| **[Facade Correlation Matrix](analysis/output/figures/nlp/facade_correlations_heatmap.png)** | Text features × psychological measures (99 cells) |
| **[UMAP Embeddings](analysis/output/figures/python/umap_three_models.png)** | Semantic space of justifications across three embedding models |
| **[Prototype Sensitivity](analysis/output/figures/python/prototype_sensitivity_master.png)** | Multiverse across 5 prototype sets × 3 embedding models |
| **[Cross-Method Convergence](analysis/output/figures/python/cross_method_convergence_heatmap.png)** | Pairwise agreement among five classifiers |
| **[Word Clouds by Hostile Tertile](analysis/output/figures/python/wordclouds_by_tertile.png)** | Lexical content by hostile-aggression group |
| **[TOST Equivalence](analysis/output/figures/supplementary/tost_equivalence_plot.png)** | Equivalence testing for null facade correlations |

### Key Data Tables

| Table | Description |
|-------|-------------|
| **[H1 Correlations](analysis/output/tables/main/H1_punitiveness_correlations.csv)** | Punitiveness × each correlate, with FDR |
| **[Construct-Level Correlations](analysis/output/tables/main/correlation_table_construct_level.csv)** | Full construct correlation matrix |
| **[Reliability](analysis/output/tables/main/reliability_alphas.csv)** | Cronbach's α for all scales (8-item and 9-item punitiveness) |
| **[Descriptives](analysis/output/tables/main/descriptive_statistics.csv)** | Mean, SD, median for all composite measures |
| **[Demographic Moderation](analysis/output/tables/supplementary/interactions_punitiveness_agg.csv)** | All 48 interaction tests, FDR-corrected |
| **[Tautology Sensitivity](analysis/output/tables/supplementary/tautology_steiger_by_level.csv)** | H2 robustness across three stringency levels |
| **[Multiverse Verdicts](analysis/output/tables/nlp/mismatch_multiverse.csv)** | Six prosocial-dark gap measures vs. cultural-default/individual-facade/sincerity predictions |
| **[Facade Matrix](analysis/output/tables/nlp/facade_correlations_full.csv)** | All 99 text-feature × psychological-measure correlations |
| **[Cross-Method Convergence](analysis/output/tables/nlp/cross_method_convergence_5way.csv)** | Pairwise raw agreement and Cohen's κ |
| **[Prototype Sensitivity](analysis/output/tables/python/prototype_sensitivity_master.csv)** | All 45 prototype-set × embedding-model combinations |
| **[Hierarchical Regression](analysis/output/tables/nlp/regression_punitiveness_hierarchical.csv)** | Text features add ΔR² over Study 1 scales |
...
│   │   └── 04_topics_convergence_export.ipynb  # BERTopic, convergence, export
│   ├── output/
│   │   ├── tables/
│   │   │   ├── main/             # Study 1 tables (~17 CSVs)
│   │   │   ├── nlp/              # Study 2 tables (~16 CSVs)
│   │   │   ├── supplementary/    # Sensitivity analyses (~23 CSVs)
│   │   │   └── python/           # Direct outputs from Python notebooks
│   │   └── figures/
...
│   │   └── PUNISHMENT_21_-__Pro-Social.qsf   # Importable into Qualtrics
...
#    Each notebook can be opened in Jupyter, Google Colab, or run from the command line.
#    Colab links are listed in the table at the top of this README.
...
jupyter notebook analysis/python/04_topics_convergence_export.ipynb
...
| 3 | `03_embeddings_and_similarity.ipynb` | BGE, voyage, MPNet embeddings × 5 prototype sets |
| 4 | `04_topics_convergence_export.ipynb` | BERTopic; UMAP; cross-method convergence; final feature export |

All four notebooks are also available as **Open in Colab** links from the table at the top of this README — useful if you want to run the pipeline without setting up local GPU and API access.
