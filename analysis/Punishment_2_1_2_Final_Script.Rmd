...
if(nrow(low_alpha) > 0) {
  cat("\n WARNING: Composites with Î± < .60 (interpret with caution): \n")
...
descriptives_table <- df_clean %>%
  select(all_of(construct_vars_desc)) %>%
...
## 5.3 Percentage Above/Below Midpoint

```{r midpoint_analysis}
cat("=== PERCENTAGE ABOVE SCALE MIDPOINT ===\n\n")
cat("(Midpoint = 4 for 1-7 scales; 25 for 0-50 sentence scale)\n\n")

# Function to compute % above midpoint
pct_above_midpoint <- function(x, midpoint = 4) {
  mean(x > midpoint, na.rm = TRUE) * 100
}

# Composites (1-7 scale, midpoint = 4)
scale_vars <- construct_vars_desc[!construct_vars_desc %in% c("Sentence_1", "punitiveness_agg")]

midpoint_table <- df_clean %>%
  select(all_of(scale_vars)) %>%
  summarise(across(everything(), ~pct_above_midpoint(., midpoint = 4))) %>%
  pivot_longer(everything(), names_to = "Construct", values_to = "Pct_Above_Midpoint") %>%
  arrange(desc(Pct_Above_Midpoint)) %>%
  mutate(Pct_Above_Midpoint = sprintf("%.1f%%", Pct_Above_Midpoint))

print(as.data.frame(midpoint_table), row.names = FALSE)

# Sentence (0-50 scale, midpoint = 25)
cat(sprintf("\nSentence_1 (midpoint = 25): %.1f%% above midpoint\n",
            pct_above_midpoint(df_clean$Sentence_1, midpoint = 25)))
cat("\n")
```

## 5.4 Sentence Distribution
...
## 5.5 Sentence Distribution Visualizations
...
if(is.null(icc_result) || is.atomic(icc_result)) {
  ...
  cat("(Model showed singularity - random effect variance â‰ˆ 0)\n\n")
...
cat("Unique Variance Explained (srÂ²):\n")
...
## 10.3 Hierarchical Regression (Incremental RÂ²)
...
cat(sprintf("Step 1 (Crime Concerns):     RÂ² = %.3f\n", summary(step1)$r.squared))
cat(sprintf("Step 2 (+Hostile):           RÂ² = %.3f, Î”RÂ² = %.3f\n", 
            summary(step2)$r.squared, 
            summary(step2)$r.squared - summary(step1)$r.squared))
cat(sprintf("Step 3 (+Emotions):          RÂ² = %.3f, Î”RÂ² = %.3f\n", 
            summary(step3)$r.squared,
            summary(step3)$r.squared - summary(step2)$r.squared))
cat(sprintf("Step 4 (+Personality):       RÂ² = %.3f, Î”RÂ² = %.3f\n\n", 
...
cat(sprintf("  Total RÂ² (all clusters): %.3f\n", summary(cluster_reg)$r.squared))
